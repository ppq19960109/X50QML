...
function makeRequest()
{
    ...
    doc.onreadystatechange = function() {
        if (doc.readyState === XMLHttpRequest.HEADERS_RECEIVED) {
            console.log("HEADERS_RECEIVED Headers -->",doc.status,doc.statusText);
...
function weatherRequest(city)
{
    ...
    doc.onreadystatechange = function() {
        if (doc.readyState === XMLHttpRequest.HEADERS_RECEIVED) {
            ...
            //            console.log("weather response",curTemp,curMinTemp,curMaxTemp)
        }
    }
...
function setMultiCooking(list,orderTime,cookPos,dishName,cookbookID)
{
    ...
    for(var i = 0; i < list.length; ++i)
    {
        ...
        if(step.iceSteamID!=null)
            buf.IceSteamID=step.iceSteamID
