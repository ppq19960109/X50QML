...
QmlDevState::QmlDevState(QObject *parent) : QObject(parent)
{
    ...
    setState("StOvState",0);
    setState("StOvMode",0);
    setState("StOvSetTemp",0);
    setState("StOvSetTimer",0);

    setState("RStOvState",0);
    setState("RStOvSetTemp",0);
    setState("RStOvSetTimer",0);

    setState("cnt",0);
    setState("current",0);
    setState("remind",false);
    setState("RemindText","");

    setState("RStoveTimingState",0);
    setState("RStoveTimingLeft",110);
...
int QmlDevState::insertHistory(QVariantMap& single)
{
    ...
    if(compareHistoryCollect(single)>=0)
    {
        QJsonObject Data;
        QJsonObject item;
